...
export default async function BlogPostPage({
  ...
  const blog = dict.blog as { back: string; all: string };
          ...
          <div className={SECTION.CONTAINER}>
            ...
            <div className="relative max-w-3xl">
              <div className="flex flex-wrap gap-2 mb-5">
                {post.tags.map((tag) => (
                  <span key={tag} className="px-2.5 py-1 text-[11px] font-medium bg-white/15 backdrop-blur-sm text-white rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
